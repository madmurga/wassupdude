...
def ban_account(username: str, reason: str = "") -> tuple[bool, str]:
    user_key = normalize_username(username)
    data = load_account_data()
    account = data["users"].get(user_key)
    ...
    account["banned"] = True
    account["banned_at"] = _now_iso()
    account["ban_reason"] = reason.strip()
    save_account_data(data)
    return True, f"{account.get('username', user_key)} has been banned."


def unban_account(username: str) -> tuple[bool, str]:
    user_key = normalize_username(username)
    data = load_account_data()
    account = data["users"].get(user_key)
    ...
    account["ban_reason"] = ""
    save_account_data(data)
    return True, f"{account.get('username', user_key)} has been unbanned."


def get_admin_account_rows() -> list[dict]:
    data = load_account_data()
    rows = []
